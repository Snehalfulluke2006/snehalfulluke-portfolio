...
function StatusIndicator({ status }: { status: "Operational" | "Degraded" | "Offline" }) {
    const colors = {
        Operational: "bg-emerald-400 text-emerald-400 uppercase tracking-widest font-black text-[10px]",
        Degraded: "bg-amber-400 text-amber-400 uppercase tracking-widest font-black text-[10px]",
        Offline: "bg-red-500 text-red-500 uppercase tracking-widest font-black text-[10px]"
    }
    return (
        <div className="flex items-center gap-3">
            <div className={`w-2 h-2 rounded-full ${colors[status].split(" ")[0]} animate-pulse`} />
            <span className={colors[status].split(" ").slice(1).join(" ")}>{status}</span>
        </div>
    )
}

export default function StudioDashboardClient({ initialData, initialHealth }: { initialData: any, initialHealth: any }) {
    const [data, setData] = useState(initialData)
    const [health, setHealth] = useState(initialHealth)

    useEffect(() => {
        const interval = setInterval(async () => {
            const [newData, newHealth] = await Promise.all([
                getAnalyticsSummary(),
                getHealthStatus()
            ])
            setData(newData)
            setHealth(newHealth)
        }, 15000) // Poll every 15s for "Realtime" feel without melting quota
            ...
            <div className="flex justify-between items-end">
                <div>
                    ...
                    <p className="text-white/40 mt-1 text-sm">
                        Welcome back. Here&apos;s what&apos;s happening in real time.
                    </p>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="p-8 rounded-[32px] glass border-white/5 space-y-5 hover:border-white/10 transition-all">
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-center">
                                <span className="text-xs font-bold text-white/60">Edge Network</span>
                                <StatusIndicator status={health.edge} />
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-xs font-bold text-white/60">Authentication</span>
                                <StatusIndicator status={health.auth} />
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-xs font-bold text-white/60">Database</span>
                                <StatusIndicator status={health.db} />
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-xs font-bold text-white/60">Storage Bucket</span>
                                <StatusIndicator status={health.storage} />
                            </div>
                        </div>
